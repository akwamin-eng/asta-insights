...
// 🟢 NEW IMPORTS
import DevelopmentCostCalculator from "../components/intel/DevelopmentCostCalculator";
import GridConnectionCard from "../components/intel/GridConnectionCard";
...
export default function ListingPage() {
  ...
  // 🟢 INTELLIGENT CONTEXT SWITCH
  const isLand = property.property_class === "Land";
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-emerald-500 selection:text-black">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-12">
        ...
        <div className="space-y-12">
          {/* 🟢 DYNAMIC STATS GRID: Land vs Structure */}
            ...
            {isLand ? (
              ...
              <>
                <div className="flex flex-col items-center justify-center p-4 bg-white/5 rounded-lg border border-white/5">
                  <Ruler size={24} className="text-blue-500 mb-2" />
                  <span className="text-xl font-bold font-mono">
                    {property.land_metadata?.acres?.toFixed(2) || "-"}
                  </span>
                ...
                <div className="flex flex-col items-center justify-center p-4 bg-white/5 rounded-lg border border-white/5">
                  ...
                  <span className="text-lg font-bold capitalize text-center">
                    {(property.land_metadata?.title_type || "Indenture").replace("_", " ")}
                  </span>
                  <span className="text-[10px] text-gray-500 uppercase tracking-widest">
                    Title
                  </span>
                ...
                <div className="flex flex-col items-center justify-center p-4 bg-white/5 rounded-lg border border-white/5">
                  <Mountain size={24} className="text-orange-500 mb-2" />
                  <span className="text-lg font-bold capitalize text-center">
                    {property.land_metadata?.topography || "Flat"}
                  </span>
                  <span className="text-[10px] text-gray-500 uppercase tracking-widest">
                    Terrain
                  </span>
          ...
          {/* 🟢 NEW: LAND INTELLIGENCE MODULES */}
          {isLand && property.land_metadata && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4">
              <DevelopmentCostCalculator 
                acres={property.land_metadata?.acres || property.land_metadata?.calculated_acres || 0.16} 
                topography={property.land_metadata?.topography || 'flat'} 
              />
              <GridConnectionCard locationName={property.location_name} />
            </div>
          )}
