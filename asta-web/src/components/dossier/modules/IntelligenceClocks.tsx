...
// ... (Keep ClockCard component EXACTLY as is) ...
...
function ClockCard({
  ...
  return (
    <div
      className={`bg-white/5 border border-white/10 p-3 rounded-lg relative overflow-hidden group hover:border-${color}-500/30 transition-colors`}
    >
...
export default function IntelligenceClocks() {
  ...
  useEffect(() => {
    async function getVolume() {
      // Count total intelligence points
            ...
            <div className="flex gap-0.5">
              <div
                className={`w-6 h-1 rounded-full ${
                  marketVolume > 200 ? "bg-orange-500" : "bg-gray-700"
                }`}
              />
              <div
                className={`w-6 h-1 rounded-full ${
                  marketVolume > 500 ? "bg-orange-500" : "bg-gray-700"
                }`}
              />
              <div
                className={`w-6 h-1 rounded-full ${
                  marketVolume > 800 ? "bg-orange-500" : "bg-gray-700"
                }`}
              />
            </div>
