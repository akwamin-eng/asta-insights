...
const GlassButton = ({
  ...
  icon: any;
  active?: boolean;
  color?: "default" | "red" | "purple" | "emerald" | "blue";
  tooltip: string;
}) => {
  const baseStyles =
    "w-10 h-10 rounded-full backdrop-blur-xl border flex items-center justify-center transition-all duration-300 shadow-lg active:scale-90 active:shadow-inner";

  const colorStyles = {
    default: active
      ? "bg-white text-black border-white shadow-[0_0_15px_rgba(255,255,255,0.3)]"
      : "bg-black/40 text-white border-white/10 hover:bg-white/10 hover:border-white/30",
    red: active
      ? "bg-red-500 text-white border-red-400 shadow-[0_0_15px_rgba(239,68,68,0.4)]"
      : "bg-black/40 text-red-400 border-white/10 hover:bg-red-500/10 hover:border-red-500/30",
    purple: active
      ? "bg-purple-500 text-white border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.4)]"
      : "bg-black/40 text-purple-400 border-white/10 hover:bg-purple-500/10 hover:border-purple-500/30",
    emerald: active
      ? "bg-emerald-500 text-white border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.4)]"
      : "bg-black/40 text-emerald-400 border-white/10 hover:bg-emerald-500/10 hover:border-emerald-500/30",
    blue: active
      ? "bg-blue-500 text-white border-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.4)]"
      : "bg-black/40 text-blue-400 border-white/10 hover:bg-blue-500/10 hover:border-blue-500/30",
  };
  ...
  return (
    <button
      ...
      className={`${baseStyles} ${colorStyles[color]} group relative`}
      title={tooltip}
    >
...
// 🟢 ARCHITECTURAL CONSTANTS FOR SIDEBAR GEOMETRY
...
export default function AstaMap() {
  ...
  const handleReset = () => {
    ...
    setSearchParams({});
    const initial = { longitude: -0.187, latitude: 5.6037, zoom: 11 };
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 🟢 FIXED: Safe filter prevents crash on null/undefined locations
    const localMatches = Array.from(
      new Set(
        listings
          .map((l) => l.location_name)
          .filter((name): name is string => !!name) // Filter out nulls/undefined
      )
    )
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((l) => {
      ...
      const isMyPending =
        status === "pending_review" && user?.id === l.owner_id;
  ...
  const sidebarVariants = {
    desktop: {
      x: isSidebarHovered ? 0 : -CONTENT_WIDTH,
      opacity: 1,
      transition: { type: "spring", stiffness: 400, damping: 40 },
    },
    mobile: {
      y: isMobileExpanded ? 0 : "calc(100% - 100px)",
      opacity: 1,
      transition: { type: "spring", stiffness: 400, damping: 40 },
    },
  };
...
cursor: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="%2310b981" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 22h20L12 2z"/><circle cx="12" cy="14" r="2"/></svg>') 16 16, auto !important;
}
...
`}</style>
      ...
      <AnimatePresence>
        {showTutorial && <MapOnboarding onComplete={completeTutorial} />}
      </AnimatePresence>
      ...
      <Map
        ...
        maxBounds={GHANA_BOUNDS as any}
        renderWorldCopies={false}
        onLoad={() => setTimeout(() => setIsMapLoaded(true), 1000)}
        onContextMenu={(e) => {
          e.preventDefault();
      ...
      >
            ...
            <div className="flex flex-col items-center gap-1 group cursor-grab active:cursor-grabbing">
              <div className="bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/20 text-white shadow-xl mb-1 flex items-center gap-2">
                ...
                <button
                  ...
                  className="bg-red-600/20 hover:bg-red-600/40 text-red-400 border border-red-500/30 rounded p-1 transition-colors"
        ...
        <div className="map-controls-panel absolute top-4 right-4 md:bottom-32 md:top-auto md:right-[10px] flex flex-col gap-4 z-10 items-end">
          <motion.button
            ...
            className="add-listing-btn group relative w-14 h-14 rounded-full bg-emerald-500/90 backdrop-blur-2xl border border-white/20 shadow-[0_0_40px_rgba(16,185,129,0.4)] flex items-center justify-center text-white overflow-hidden active:shadow-inner"
          >
          ...
          <div className="flex flex-col gap-2 p-2 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 shadow-2xl">
            <GlassButton
              ...
              icon={gpsLoading ? Loader2 : Crosshair}
              color="emerald"
              active={gpsLoading}
            ...
            <GlassButton
              ...
              icon={Shield}
              active={showTrustRadar}
              color="purple"
              tooltip="Verified Trust Radar"
            />
            <div className="w-full h-px bg-white/10 my-0.5" />
            <GlassButton
        ...
        >
          {/* CONTENT SECTION (Hidden when retracted) */}
          <div
            style={{ width: CONTENT_WIDTH }}
            className="flex flex-col bg-asta-deep/95 backdrop-blur-md h-full shrink-0 overflow-hidden"
          >
            <div
            ...
            <div className="p-3 border-b border-white/10">
                ...
                <div className="ml-auto">
                  {user ? (
                    <button
                      ...
                      className="text-[9px] font-bold text-emerald-400 border border-emerald-500/30 px-2 py-1 rounded hover:bg-emerald-500/10 transition-all flex items-center gap-1"
                    >
                      <LayoutDashboard size={10} /> DOSSIER
                    </button>
                  ) : (
                    <button
              ...
              <div className="relative mb-3 z-50">
                <div className="search-bar-container relative">
                  <input
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-lg py-1.5 pl-3 pr-8 text-xs text-white focus:outline-none focus:border-emerald-500/50 transition-all font-mono"
                  />
                  {isSearching || autocompleteLoading ? (
                    <Loader2 className="absolute right-3 top-2 text-emerald-500 w-3 h-3 animate-spin" />
                  ) : (
                    <Search className="absolute right-3 top-2 text-gray-500 w-3 h-3" />
                  )}
                </div>
                {showSuggestions && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-black/90 border border-white/10 rounded-lg shadow-xl overflow-hidden backdrop-blur-md">
                    {suggestions.map((item, i) => (
                      <div
                        ...
                        className="flex items-center gap-3 px-3 py-2 hover:bg-white/10 cursor-pointer transition-colors border-b border-white/5 last:border-0"
                      >
                        {item.type === "location" ? (
              ...
              >
                <div className="flex flex-wrap gap-1.5 mb-3 max-h-20 overflow-y-auto scrollbar-hide">
                  {trendingTags.map((tag, i) => (
                    <button
                      ...
                      className="px-2 py-0.5 bg-white/5 border border-white/10 rounded text-[9px] text-asta-platinum hover:bg-emerald-500/20 transition-all"
                    >
                      #{tag}
                    </button>
                  ))}
                ...
                <div className="flex gap-1 p-1 bg-white/5 rounded-lg mb-3">
                  {["all", "sale", "rent"].map((type) => (
                    <button
                      ...
                      className={`flex-1 py-1 text-[9px] uppercase font-bold tracking-wider rounded transition-all ${
                        filterType === type
                ...
                <div className="mb-2">
                  <div className="flex items-center gap-2 mb-2 bg-white/5 p-1 rounded-lg border border-white/10">
                    <input
                      ...
                      className="w-full bg-transparent text-white text-[10px] pl-2 py-1 focus:outline-none placeholder-gray-600"
                    />
                  ...
                  <div className="flex gap-1">
                    {["Budget", "Family", "Luxury"].map((label) => (
                      <button
                        ...
                        className="flex-1 py-1 text-[8px] border border-white/10 rounded hover:bg-white/10 text-gray-400 transition-colors uppercase tracking-wide"
                      >
                        {label}
                      </button>
                    ))}
                ...
                <button
                  ...
                  className="w-full mt-1 flex items-center justify-center gap-2 py-1.5 rounded border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 hover:border-red-500/50 transition-all text-[9px] uppercase font-bold tracking-widest"
                >
          ...
          {/* HANDLE SECTION (Always visible) */}
          <div
            style={{ width: HANDLE_WIDTH }}
            className="hidden md:flex h-full items-center justify-center cursor-pointer group bg-asta-deep/90 border-l border-white/5"
          >
          ...
          {showEmptyState && (
            ...
            >
              ...
              <div className="flex gap-2 mt-2 w-full">
                <button
                  onClick={() => {
                    const loc = showEmptyState.location;
                    setShowEmptyState(null);
                    if (user) {
                      setSearchParams({
                        mode: "dossier",
