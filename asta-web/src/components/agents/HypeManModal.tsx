...
// ⚠️ We use the environment variable, or fallback to your provided key
const API_KEY =
  import.meta.env.VITE_GOOGLE_API_KEY ||
  "AIzaSyCUQ3Oy0kxzvIT2HaQdSZq3Dst7cf5-ZxQ";
...
export default function HypeManModal({ property, onClose }: HypeManProps) {
  ...
  // --- 🕵️ MODEL HUNTER (Debugs your API Key) ---
  useEffect(() => {
    const listActiveModels = async () => {
      try {
        console.log("🕵️ Hypeman: Asking Google for active models...");
        // Direct fetch because the SDK sometimes obscures the list method
        ...
        if (data.models) {
          ...
          // We don't show this to user, just for your F12 Console debugging
        } else {
          console.error("❌ NO MODELS FOUND. API RESPONSE:", data);
          if (data.error) {
            setDebugLog(`API Error: ${data.error.message}`);
          }
        }
      } catch (e) {
        console.error("❌ Network Error checking models:", e);
      }
    };
    listActiveModels();
  }, []);
  ...
  const generateStrategies = async () => {
    try {
      ...
      // 🟢 CHANGE: Trying the explicit '001' version which is often more stable
      // If this fails, check your F12 console to see which models you actually have access to.
      ...
      return [
        {
          ...
          text: aiContent.option_1,
        },
        {
          platform: "instagram",
          icon: Instagram,
          label: 'The "Aesthetic" Caption',
          text: aiContent.option_2,
        },
        {
          platform: "linkedin",
          icon: Linkedin,
          label: 'The "Investor" Pitch',
          text: aiContent.option_3,
        },
      ];
    } catch (error: any) {
      console.error("Hypeman Brain Freeze:", error);
      // Capture the error message to show in the UI
      setDebugLog(error.message || "Unknown API Error");
      return [];
    }
  };
          ...
          <AnimatePresence mode="wait">
            ...
            {step === "error" && (
              ...
              >
                ...
                {/* 🔍 DEBUG DISPLAY FOR YOU */}
                <div className="bg-red-500/10 border border-red-500/20 p-3 rounded-lg max-w-sm mb-4">
                  ...
                  <p className="text-[10px] font-mono text-red-300 text-left break-all">
                    {debugLog || "Check F12 Console for active model list."}
                  </p>
                </div>
            ...
            {step === "done" && (
              ...
              >
                {variations.map((v, i) => (
                  <div
                    key={i}
                    className="bg-white/5 border border-white/10 rounded-xl p-4 hover:border-purple-500/30 transition-colors group"
                  >
