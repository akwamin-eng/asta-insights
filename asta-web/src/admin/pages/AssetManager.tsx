...
export default function AssetManager() {
  ...
  async function fetchAssets() {
    setLoading(true);
    // Fetch properties with related owner profile AND partner details
    const { data, error } = await supabase
      .from("properties")
      .select(
        "*, profiles:owner_id(full_name, email), partners(name, commission_rate)"
      )
  ...
  const handleVerifyToggle = async (id: number, currentStatus: boolean) => {
    setAssets((prev) =>
      prev.map((a) => (a.id === id ? { ...a, verified: !currentStatus } : a))
    );
  ...
  const filteredAssets = assets.filter((asset) => {
    const matchesSearch =
      ...
      asset.partners?.name?.toLowerCase().includes(search.toLowerCase()) ||
      asset.details?.external_id?.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-xs">
            Total Database: {assets.length} Records
          </p>
        </div>
      ...
      <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            {/* 🟢 FIX: Removed comments inside TR to prevent hydration errors */}
            <thead>
              <tr className="bg-white/5 border-b border-white/10 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                ...
                <th className="p-4 w-24">Ref ID</th>
                  ...
                  >
                      ...
                      {asset.partners ? (
                        ...
                        <div>
                          <div className="text-xs text-white">
                            {asset.profiles?.full_name || "Unknown"}
                          </div>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() =>
                            window.open(`/listing/${asset.id}`, "_blank")
                          }
                        ...
                        <button
                          onClick={() =>
                            handleStatusChange(
                              asset.id,
                              asset.status === "active" ? "draft" : "active"
                            )
                          }
